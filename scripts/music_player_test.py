...
class MusicPlayer:
    ...
    def loop(self):
        if self.is_song_loaded() &&
           (self.channel.get_position() == self.sound.get_length()):
                self.on_end_song();
    ...
    def get_song_volume(self):
        if(song.is_song_loaded()):
            return int(self.channel.get_volume())
        else:
            return 0.0
    ...
    def load_song(self, song):
        if(self.is_song_loaded() && self.is_playing):
            self.channel.stop()
    ...
    def seek_to(self, seekTime):
        if self.is_song_loaded():
            self.chanel.set_position(float(args[0]))

    def is_song_loaded(self):
        return self.sound != None && self.channel != None;
